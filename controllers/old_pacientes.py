...
def buscar(base, id):
    bases = ['paciente', 'agendamento', 'consulta', 'consulta_paciente']
...
def pacientes():
    links = [lambda row: A(SPAN('Visualizar',
        _class='icon magnifier icon-zoom-in glyphicon glyphicon-zoom-in'),
        _class='button btn btn-default',
        _href=URL(c='pacientes', f='paciente', args=[row.id])),
            lambda row: A(SPAN('Editar',
                _class='icon pen icon-pencil glyphicon glyphicon-pencil'),
                _class='button btn btn-default',
                _href=URL(c='pacientes', f='editar_cadastro', args=[row.id]))]
    grid = SQLFORM.grid(db.pacientes,
        fields=[db.pacientes.nome, db.pacientes.nascimento, db.pacientes.sexo],
        csv=False, links=links, details=False, create=False, editable=False,
        deletable=True)
    return locals()
...
def novo_agendamento():
    links = [lambda row: A(SPAN('Agendar',
        _class='icon plus icon-plus glyphicon glyphicon-plus'),
        _class='button btn btn-default', _href=URL(c='pacientes', f='agendar',
            args=[row.id]))]
    grid = SQLFORM.grid(db.pacientes,
            fields=[db.pacientes.nome, db.pacientes.cpf],
        links=links, csv=False, editable=False, deletable=False,
...
def agendar():
    id_paciente = request.args(0) or redirect(URL(c='pacientes',
        f='novo_agendamento'), client_side=True)
    paciente = buscar('paciente', id_paciente)
...
def editar_agendamento():
    id_agendamento = request.args(0) or redirect(URL(c='pacientes',
        f='agendamentos'), client_side=True)
...
def agendamento():
    ...
    form = SQLFORM.factory(
            Field('tipo_consulta',
                requires=IS_IN_SET([i['label'] for i in tipos_consultas])))
    if form.process().accepted:
        ...
        redirect(URL(c='pacientes', f='consulta',
            args=[tipo_consulta, id_agendamento], vars=dict(agendamento=True)),
            client_side=True)
    return locals()
...
def consulta():
    ...
    response.view = tipo_consulta['view']
...
def nova_consulta():
    links = [lambda row: A('Iniciar consulta', _class='button btn btn-default',
        _href=URL(c='pacientes', f='consultar', args=[row.id]))]
    grid = SQLFORM.grid(db.pacientes,
            fields=[db.pacientes.nome, db.pacientes.cpf],
            links=links, csv=False, editable=False, deletable=False,
            details=False, create=False)
...
def todas_consultas():
    links = [lambda row: A('Ver consulta', _class='button btn btn-default',
            _href=URL(c='pacientes', f='ver_consulta', args=[row.id]))]
    form = SQLFORM.smartgrid(db.consultas, linked_tables=['id_paciente'],
            fields=[db.consultas.id_paciente, db.consultas.tipo_consulta,
                db.consultas.hora_fim],
            csv=False, editable=False, deletable=False, details=False,
            create=False, links=links)
    return locals()


def ver_consulta():
    id_consulta = request.args(0) or \
            redirect(URL(c='pacientes', f='todas_consultas'))
